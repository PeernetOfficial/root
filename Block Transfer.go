...
func blockPrintProfileField(field blockchain.BlockRecordProfile, output io.Writer) {
	...
	case blockchain.ProfilePicture:
		...
		printPicture(field.Data, output)
...
func printPicture(data []byte, output io.Writer) {
	// Create convert options
	convertOptions := convert.DefaultOptions
	convertOptions.FixedWidth = 50
	convertOptions.FixedHeight = 20

	imageData, format, err := image.Decode(bytes.NewBuffer(data))
	if err != nil {
		fmt.Fprintf(output, "Cannot decode picture: %v\n", err.Error())
		return
	}
	fmt.Fprintf(output, "Decoded picture as format %s:\n", format)

	// Create the image converter
	converter := convert.NewImageConverter()
	output.Write([]byte(converter.Image2ASCIIString(imageData, &convertOptions)))
}
