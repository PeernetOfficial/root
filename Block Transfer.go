...
func printPicture(data []byte) {
	...
	if err != nil {
		fmt.Printf("Cannot decode picture: %v\n", err.Error())
		return
	}
	fmt.Printf("Decoded picture as format %s:\n", format)

	// Create the image converter
	converter := convert.NewImageConverter()
	fmt.Print(converter.Image2ASCIIString(imageData, &convertOptions))
